...
int main() {
    {
        Arc<rdma_util::MemoryRegion> mr = nullptr;

        {
...
#if USE_CUDA
            auto length = 1024;
            Arc<void> buffer_with_deleter(gpu_mem_util::malloc_gpu_buffer(1024, 7), [](void* p) {
            ...
            Arc<void> buffer_with_deleter(new rdma_util::Ticket[128], [](rdma_util::Ticket* p) {
